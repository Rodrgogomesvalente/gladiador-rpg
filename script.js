...
const MERCS = [
    ...
    { id: 3, name: "Centurião Renegado", cost: 1000, bonusSkl: 10, bonusStr: 10 }
];
...
function battle() {
    ...
    if(Math.random() * 25 < tSkl) {
        ...
        player.gold += win;
        addLog(`⚔️ Vitória! Ganhou ${win} de ouro.`, "success");
    } else {
        addLog("⚔️ Derrota na arena...", "error");
    }
    updateUI();
}
...
function startJob() {
    ...
    if(hrs >= 1 && hrs <= 8) {
        ...
        addLog(`💼 Você começou a trabalhar por ${hrs}h.`, "info");
        updateUI();
    }
}
...
function updateJobProgress() {
    ...
    if(now >= player.job.endTime) {
        document.getElementById('job-msg').innerText = "Trabalho Concluído!";
...
function collectJob() {
    ...
    addLog(`💰 Recebeu ${player.job.reward} de ouro do trabalho.`, "success");
...
function hire(id) {
    ...
    if(player.gold >= m.cost) {
        ...
        addLog(`🤝 ${m.name} contratado!`, "success");
        updateUI();
    }
}
